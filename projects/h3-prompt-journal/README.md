...
页面另提供 CASE 001–005 的五套真实前置信息：Codex 内置 ImageGen 生成的 11 张原创参考图、与上方默认模板完全一致的动态双语 Prompt、`MiniMax-H3 / Ref2VA / 12–15s / 768P` 参数，以及逐套原生立体声方案。五套外部 MP4 现已全部返回；页面保留原始 V1、成功项和失败项，不用修复意图覆盖真实结果。完整素材说明见 [`real-samples/`](real-samples/README.md)。
...
CASE 002 的双人 3:1 差速成片已返回：`12.25s / 1344×768 / H.264 / AAC stereo`。下载文件为正常 16:9，预览中的模糊侧边与水印没有写入文件；但 25 个均匀抽样画面全部裁掉双脚，并在约 `3.21 / 9.71s` 切镜，所以蝴蝶步和 3:1 比例根本不可观察。研究记录将主要原因定位为 V1 同时要求全身动作与腰部 / 膝盖以上近距摄影的语义冲突，并提供不覆盖 V1、尚待验证的轻量 V2 双语稿：固定全身镜头、一个可数动作周期、快者三次 / 慢者一次、声音同步计数。
