...
Required artifacts: 项目研究 README、分析文档、中英双版本生产 Prompt 设计器、CASE 001–005 五套真实测试包与 11 张参考图、逐套原生立体声方案、CASE 006 外部真实 MP4 与客观偏差记录、真实输出区持续可见的输入图与输出视频对照、总索引入口、浏览器验收记录
...
Observable completion criteria: 六种模板均能切换英文生产版与中文理解版；CASE 001–005 样例 Prompt 与对应模板恢复默认后的输出逐字一致；11 张参考图均可加载和下载；每套明确原生立体声、动作声、音乐或对白关系；CASE 006 返回 MP4 可播放且音轨存在，页面同时呈现通过项与偏差；加入输出视频后输入图仍独立、持续可见，视频封面来自输出成片而非输入图；三个视口不溢出；缺少输出的案例不得冒充实测
```
...
| 真实样例 | CASE 001–005 共 11 张输入图、动态双语 Prompt 与参数 | 五张样例卡、图片下载、Prompt 下载 | 11 图浏览器加载与 HTTP 200；10 个按钮存在；中英文均与默认模板逐字一致 | 5-9 | pass | 用户外部生成后再补真实 MP4，不作为前置信息交付阻塞 |
